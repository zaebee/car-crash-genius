      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                <div className="absolute top-full left-0 w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-20 flex flex-col p-1 animate-in fade-in zoom-in-95 duration-200">
                  {AVAILABLE_MODELS.map((model) => (
                    <button
                      ...
                      className={`flex items-center gap-3 p-3 rounded-lg transition-colors text-left group ${selectedModel.id === model.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                        }`}
                    >
          ...
          <button
            ...
            className={`w-full py-4 px-4 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-lg ${isGenerating || (files.length === 0 && !textInput.trim())
